...
const Login = () => {
    ...
    const onButtonClick = () => {
        fetch('post', '/login', {...subject, username: username}).then(res => {
            if (res.status === 200) {
                setInvalid(false);
                setSubjectAsyc({...subject, username: username, helpType: res.data.helpType, subject: res.data.subject}).then(() => {
                    navigate('/master-study-system');
                });
            }
        }).catch(err => {
    ...
    return (
        <div>
            <div id="loginForm">
                ...
                {invalid && <p style={{color: "red", marginTop: "2%"}}>{responseMsg}</p>}
            </div>
        </div>
    )
}
